...
def winograd_matrix_mult(matrix1, matrix2):
    ...
    for i in range(q):
        for j in range(0, m // 2, 1):
            column_factor[i] = column_factor[i] + matrix2[2 * j][i] * matrix2[2 * j + 1][i]

    for i in range(n):
        for j in range(q):
            matrix_res[i][j] = -row_factor[i] - column_factor[j]
            for k in range(0, m // 2, 1):
                matrix_res[i][j] = matrix_res[i][j] + (matrix1[i][2 * k + 1] + matrix2[2 * k][j]) * \
                                                      (matrix1[i][2 * k] + matrix2[2 * k + 1][j])

    if m % 2 == 1:
        for i in range(n):
            for j in range(q):
                matrix_res[i][j] = matrix_res[i][j] + matrix1[i][m - 1] * matrix2[m - 1][j]
    ...
    for i in range(n):
        for j in range(q):
            ...
            for k in range(1, m, 2):
                ...
                if (flag):
                    matrix_res[i][j] += matrix1[i][m - 1] * matrix2[m - 1][j]
